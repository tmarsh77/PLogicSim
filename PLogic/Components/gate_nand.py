...
class GateNand(SpriteRenderer, LogicGate, Serializable):
    def __init__(self, simulator):
        LogicGate.__init__(self)
        image = simulator.resources.load_image("nand.png")
        ...
        self.q = Pin.create_pin(simulator, self, Vector2(220, 42))
        self.i1.on_value_changed = self.resolve
        self.i2.on_value_changed = self.resolve
        self.simulator = simulator

    def resolve(self):
        self.q.set_signal(self, 1 if not (self.i1.signal == 1 and self.i2.signal == 1) else 0)
